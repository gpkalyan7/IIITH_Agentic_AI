...
_ASKS_OWNER = re.compile(
    r"(?i)\b(can you|could you|would you|please (review|sign|approve|confirm|share|send)"
    r"|does .{0,20}work|any read on|let us know|need your|waiting on you|resend)\b"
)
...
_LEGAL_MONEY = re.compile(
    r"(?i)\b(sign|signature|safe|term sheet|amendment|contract|counsel|llp|invoice|wire|remit|deposit|"
    r"board minutes|ip assignment)\b"
)
...
_DATED = re.compile(
    r"(?i)\b(by (the )?\d{1,2}(st|nd|rd|th)?|by (mon|tues|wednes|thurs|fri)day|before month-end"
    r"|on the \d{1,2}(st|nd|rd|th)?|renews? (on|in)|deadline|due |expires? on"
    r"|\d{1,2}:\d{2}\s?(am|pm)|\b\d{1,2}\s?(am|pm))\b"
)
